...
export class VeilleEventService {
  ...
  addVeille(veille: VeilleEvent): Observable<VeilleEvent> {
    return this.http
      .post<VeilleEvent>(this.apiurl, veille)
      .pipe(tap(), catchError(this.handleError));
  }

  updateVeille(
    veilleId: string,
    updateData: VeilleEvent
  ): Observable<VeilleEvent> {
    const url = `${this.apiurl}/${veilleId}`;
    return this.http
      .put<VeilleEvent>(url, updateData)
      .pipe(tap(), catchError(this.handleError));
  }
